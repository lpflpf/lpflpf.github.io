...
const rootURL = 'http://lpflpf.github.io/';
...
const codeBlockConfig = JSON.parse('{"maximum":100,"show":false}');
const iconsPath = `icons/`;

// values defined under config/_default/params.toml
let otherSearchableFields = ''
...
function loadActions() {
  ...
  headingNodes.forEach(function(node){
    link = createEl('a');
    icon = createEl('img');
    icon.src = 'http://lpflpf.github.io/icons/link.svg';
  ...
  function copyFeedback(parent) {
    ...
    if(!elem(`.${yanked}`, parent)) {
      ...
      icon.src = 'http://lpflpf.github.io/icons/check.svg';
